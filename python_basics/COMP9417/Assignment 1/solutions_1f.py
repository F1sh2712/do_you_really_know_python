...
lambda_1f = 0.001
matrix = np.identity(p) + 2 * lambda_1f * p * (W.T @ W) # Got from deduction
...
def gradient(beta, y, W, L, j):
    gradient_val = np.zeros_like(beta)
    regular_val = 2 * L * (W.T @ (W @ beta))
    # When index = j, ej = 1
    gradient_val[j] = beta[j] - y[j]
    
    gradient_val = gradient_val + regular_val
    return gradient_val

# Calculate L(beta_hat)
L_beta_hat = loss(beta_hat, y_var, W, lambda_1f)
...
for i in range(len(alpha_list)):
    ...
    for k in range(4):
        for j in range(p):
            grad = gradient(beta, y_var, W, lambda_1f, j)
            beta = beta - alpha * grad
            delta = loss(beta, y_var, W, lambda_1f) - L_beta_hat
